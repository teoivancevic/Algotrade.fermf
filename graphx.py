...
def get_min_vol(i, j, k, l, eij, ejk, ekl, eli):
    first = vol_mat[node[i]][node[j]] 
    second = min(first * eij, vol_mat[node[j]][node[k]])
    third = min(second * ejk, vol_mat[node[k]][node[l]])
    last = min(third * ekl, vol_mat[node[l]][node[i]]) 
    return int(min(first, second / ejk, third / ekl / ejk, last / ejk / ekl/ eli))
...
for i in range(len(node)):
    for j in range(len(node)):
        for k in range(len(node)):
            ...
            if node[j] in e_mat[node[i]] and node[k] in e_mat[node[j]] and node[l] in e_mat[node[k]] and node[i] in e_mat[node[l]]:
                #eij = graph[node[i]][eij][1] / 1e8
                #ejk = graph[node[j]][ejk][1] / 1e8
                #eki = graph[node[k]][eki][1] / 1e8
                eij = e_mat[node[i]][node[j]] / 1e8
                ejk = e_mat[node[j]][node[k]] / 1e8
                ekl = e_mat[node[k]][node[l] / 1e8

                eli = e_mat[node[l]][node[i]] / 1e8
                if eij * ejk * ekl * eli > 1.001 and get_min_vol(i, j, k,l, eij, ejk, ekl, eli) / 1e8 > 0.002:
                    print("uso")
                    #print('naso ciklus: ' + node[i] + ' ' + node[j] + ' ' + node[k] + ' omjer: ', eij * ejk * eki, 'vol: ', get_min_vol(i, j, k, eij, ejk, eki) / 1e8 )
                    ...
                    trade_vol = min(int(get_min_vol(i, j, k,l, eij, ejk, ekl, eli) / eij * 9 / 10), int(int(api.balance(user)['USDT']) * 9 / 10))
                    print('vol: ', trade_vol / 1e8)
                    ...
                    url = url + '|' + node[j] + ',' + node[k] + ',' + str(trade_vol)
                    trade_vol = int(trade_vol * ejk)
                    url = url + '|' + node[k] + ',' + node[l] + ',' + str(trade_vol)
                    trade_vol = int(trade_vol * ekl)
                    url = url + '|' + node[l] + ',' + node[i] + ',' + str(trade_vol)
                    print(url)
                    print(api.createOrders(user, secret, url))
                    print('bal: ', api.balance(user)['USDT'] / 1e8)
                    print('------')
